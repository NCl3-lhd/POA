...
std::vector<res_t> poa(const para_t* para, const graph* DAG, int beg_id, int end_id, int qid, const char* qseq, int qlen, aligned_buff_t* mpool = nullptr, bool ab_band = true); // final version
std::vector<res_t> alignment(const para_t* para, graph* DAG, minimizer_t* mm, int rid, const char* qseq, int qlen, aligned_buff_t* mpool = nullptr);
#endif