...
std::vector<res_t> POA_SIMD(para_t* para, const graph& DAG, const std::string& seq, aligned_buff_t* mpool) {
  ...
  std::vector<int> mat = para->mat;
  int match = para->match, mismatch = para->mismatch, para_m = para->m, e1 = para->gap_ext1, o1 = para->gap_open1 + e1;
  reg MATCH = _mm256_set1_epi32(match), MISMATCH = _mm256_set1_epi32(mismatch), E1 = _mm256_set1_epi32(e1), O1 = _mm256_set1_epi32(o1);
  // std::cout << mat << " " << mis << " " << o1 << " " << e1 << "\n";
  ...
  for (int i = 1; i < n; i++) {
    ...
    for (int k = 0; k < cur.in.size(); k++) {
      int p = node[cur.in[k]].rank; // rank
      const node_t& pre = node[cur.in[k]];
      int prev = NEG_INF; // -inf
      char prech = char26_table['N'];
      reg PRE_BASE = _mm256_set1_epi32(pre.base);
      for (int j = 0; j < col_size; j += reg_size) { // SIMD
        reg Mij = _mm256_load_si256((reg*)(M + i * col_size + j));
        reg Dij = _mm256_load_si256((reg*)(D + i * col_size + j));

        // M[i][j] = std::max(M[i][j], M[p][j - 1] + mat[pre.base * para_m + seq[j - 1]]); // qsource
        //j - 1;
        reg TMP = _mm256_setr_epi32(prev, M[p * col_size + j], M[p * col_size + j + 1], M[p * col_size + j + 2], M[p * col_size + j + 3], M[p * col_size + j + 4], M[p * col_size + j + 5], M[p * col_size + j + 6]); //M[p][j - 1]
        reg SEQ = _mm256_setr_epi32(prech, seq[j], seq[j + 1], seq[j + 2], seq[j + 3], seq[j + 4], seq[j + 5], seq[j + 6]);
        ...
        // D[i][j] = std::max({ D[i][j], D[p][j] + e1, M[p][j] + o1 });    // dsource
        TMP = _mm256_load_si256((reg*)(D + p * col_size + j));
        TMP = _mm256_add_epi32(TMP, E1);  //D[p][j] + e1
        Dij = _mm256_max_epi32(Dij, TMP); // max(D[i][j], D[p][j] + e1)
        TMP = _mm256_load_si256((reg*)(M + p * col_size + j));
        TMP = _mm256_add_epi32(TMP, O1);  //M[p][j] + o1
        Dij = _mm256_max_epi32(Dij, TMP); // max(D[i][j], M[p][j] + o1)
        _mm256_store_si256((reg*)(M + i * col_size + j), Mij); // M[i][j] = max
        _mm256_store_si256((reg*)(D + i * col_size + j), Dij); // D[i][j] = max
        prev = M[p * col_size + j + reg_size - 1]; // pre = M[p][j +reg_size - 1]
    ...
    for (int j = 1; j <= m; j++) {
      I[i * col_size + j] = std::max(I[i * col_size + j - 1] + e1, M[i * col_size + j - 1] + o1); // isource
      M[i * col_size + j] = std::max({ M[i * col_size + j], D[i * col_size + j], I[i * col_size + j] });  // three source
    }
