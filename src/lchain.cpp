...
static inline int32_t comput_sc(const mm128_t* ai, const mm128_t* aj, int32_t max_dist_x, int32_t max_dist_y, int32_t bw, float chn_pen_gap, float chn_pen_skip, int is_cdna, int n_seg)
{
  ...
  bool is_equal = false;
  if (dq <= 0 || dq > max_dist_x) return -INF;
  dr = (int32_t)(ai->x - aj->x);
  if (is_equal && (dr == 0 || dq > max_dist_y)) return -INF;
  dd = dr > dq ? dr - dq : dq - dr;
  if (is_equal && dd > bw) return -INF;
  if (n_seg > 1 && !is_cdna && is_equal && dr > max_dist_y) return -INF;
  dg = dr < dq ? dr : dq;
  q_span = aj->y >> 32 & 0xff;
